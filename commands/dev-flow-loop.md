...
9. Only after both gates are approved, phase-level dev-flow may auto-continue within baseline. Before starting execution, verify `openspec_artifact_ready.checker_score` ≥ 95 and `task_orchestration_ready.checker_score` ≥ 95 from dev-flow-planning; do not start implementation if either is absent or below threshold. Phase execution must provide phase-level OpenSpec/opsx, the self-contained local per-task sequence `failing test first` / `observed RED` / `minimal GREEN` / `green-only refactor`, fresh phase evidence from rerun proving commands or browser checks (`fresh evidence-before-claim`), a detailed test matrix, system-level acceptance evidence, and `phase_eval` by a checker subagent with `phase_eval_result.checker_score` recorded, checker score >= 95, and no P0/P1 finding.
